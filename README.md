...
### React · Node.js · MongoDB · Istio Service Mesh

---

## 📋 Table of Contents

- [Part 1 — Application Deployment](#-part-1--application-deployment)
  - [Step 1 — Create Namespace](#step-1--create-namespace)
  - [Step 2 — Apply Secrets](#step-2--apply-secrets)
  - [Step 3 — Deploy MongoDB](#step-3--deploy-mongodb)
  - [Step 4 — Deploy Backend](#step-4--deploy-backend)
  - [Step 5 — Deploy Frontend](#step-5--deploy-frontend)
- [Part 2 — Istio Service Mesh](#-part-2--istio-service-mesh)
  - [Step 1 — Install istioctl](#step-1--install-istioctl)
  - [Step 2 — Install Istio on the Cluster](#step-2--install-istio-on-the-cluster)
  - [Step 3 — Enable Sidecar Injection](#step-3--enable-sidecar-injection)
  - [Step 4 — Restart Deployments](#step-4--restart-deployments)
  - [Step 5 — Apply Gateway and VirtualService](#step-5--apply-gateway-and-virtualservice)
  - [Step 6 — Patch IngressGateway NodePort](#step-6--patch-ingressgateway-nodeport)
  - [Step 7 — Update Frontend Backend URL](#step-7--update-frontend-backend-url)
...
| Profile | Description |
|---------|-------------|
| `minimal` | Core only, no ingress gateway |
| `demo` | Everything enabled — best for learning ✅ |
...
```bash
# Label the namespace — Istio will auto-inject Envoy into every pod
...
```bash
kubectl port-forward service/frontend-service 3000:3000 -n three-tier --address=0.0.0.0 &
...
| Backend API | `http://<YOUR-SERVER-IP>:8080/api/tasks` |
...
```bash
# Confirm NodePort is set to 30080
kubectl get svc istio-ingressgateway -n istio-system
```
...
> Made with ❤️ by [Mandhar Patil](https://github.com/mandhar-patil)
